...
class GovBuySpider(scrapy.Spider):
    name = 'govbuy_wan_shucheng'
    urls = set()
    flag = True
    wan_shucheng_extractor = LxmlLinkExtractor(allow='/tmp/xxgklist2.shtml')  # 舒城县
    sname = set()

    def start_requests(self):
        hosts = get_govs_purchase_urls_list_by_gov_name('舒城县人民政府网')  # 获取接口数据
        for url in hosts:
    ...
    def prase(self, response):
        ...
        if len(self.urls) == 0 and self.flag:
            ...
            for i in xrange(1, max_page + 1):
                ...
                self.urls.add(url)
        links = response.xpath('//li[re:test(@class, "mc$")]//a')  # 舒城县
