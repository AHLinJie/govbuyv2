...
class GovBuySpider(scrapy.Spider):
    name = 'govbuy_wan_shouxian'
    urls = set()
    flag = True
    wan_shouxian_extractor = LxmlLinkExtractor(allow='/opennessTarget')  # 寿县
    sname = set()

    def start_requests(self):
        hosts = get_govs_purchase_urls_list_by_gov_name('寿县人民政府网')  # 获取接口数据
        for url in hosts:
    ...
    def prase(self, response):
        ...
        if len(self.urls) == 0 and self.flag:
            self.flag = False
            extractor = self.wan_shouxian_extractor
        ...
        for index, link in enumerate(links):
            ...
            record = get_crawl_page_by_host_and_project_page_url(host_id, shref)  # 查看数据库是否已经保存了记录
