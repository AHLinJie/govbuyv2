...
class GovBuySpider(scrapy.Spider):
    name = 'govbuy_wan_jinan'
    urls = set()
    flag = True
    wan_shucheng_extractor = LxmlLinkExtractor(allow='/TMP/Nav_gongkailanmu.shtml?')  # 金安区
    sname = set()

    def start_requests(self):
        hosts = get_govs_purchase_urls_list_by_gov_name('金安区人民政府网')  # 获取接口数据
        for url in hosts:
    ...
    def prase(self, response):
        ...
        if len(self.urls) == 0 and self.flag:
            ...
            max_page = int(urlparse.parse_qs(urlparse.urlparse(links[-2].url).query)['pp'][0])  # 注意这里的是倒数第二个
            template = (url[:-1] + '{0}')
