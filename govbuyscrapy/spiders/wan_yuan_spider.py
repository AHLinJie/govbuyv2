...
class GovBuySpider(scrapy.Spider):
    name = 'govbuy_wan_yuan'
    urls = set()
    flag = True
    wan_shucheng_extractor = LxmlLinkExtractor(allow='/tmp/xxgklist2.shtml?')  # 裕安区
    sname = set()

    def start_requests(self):
        hosts = get_govs_purchase_urls_list_by_gov_name('裕安区人民政府网')  # 获取接口数据
        for url in hosts:
